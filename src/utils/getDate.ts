...
export const getDate2 = (date: any) => {
  ...
  let newDate = `${date.getDate()}/${date.getMonth() + 1}/${date
    .getFullYear()
    .toString()
    .slice(2, 4)}`;
  return newDate;
...
export const getDateAndTime = (date: any) => {
  ...
  date = new Date(date);
  let hr;
  if (date.getHours() === 0) hr = 12;
  else if (date.getHours() >= 13) hr = date.getHours() - 12;
...
export const getDateAndTime2 = (date: any) => {
  if (!date) {
    return "Just now";
  }
  date = new Date(date);
